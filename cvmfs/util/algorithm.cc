...
//  this->bins_.assign(nbins + 1, 0);             // +1 for overflow bin.
  ...
  for (size_t i=0; i<=nbins; ++i){
    bins_.emplace_back(std::unique_ptr<std::atomic<int32_t>>(new std::atomic<int32_t>(0))); 
  }
...
const std::vector<std::unique_ptr<std::atomic<int32_t> > >& UTLog2Histogram::GetBins(
